...
class integration_tests(unittest.TestCase):
    ...
    def get_response(self, params, endpoint='/notes'):
    ...
    def check_time_sort(self, response):
        try:
            ...
            for resource in response:
                ...
                current_element = datetime.strptime(current_element,
                                                    '%Y-%m-%dT%H:%M:%S.%f%z')
    ...
    def query_source(self, student_id, sources, endpoint='/notes'):
        ...
        if len(sources) > 0:
            array_len = random.randint(1, len(sources))
            rand_sources = random.sample(sources, array_len)
    ...
    def query_context_type(self, student_id, context_types, endpoint='/notes'):
        ...
        array_len = random.randint(0, len(context_types))
        rand_context_type = random.sample(context_types, array_len)
